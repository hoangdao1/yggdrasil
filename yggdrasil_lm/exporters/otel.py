...
import json  #49 (line in Coconut source)
import uuid  #50 (line in Coconut source)
from datetime import datetime  #51 (line in Coconut source)
from datetime import timezone  #51 (line in Coconut source)
if _coconut.typing.TYPE_CHECKING:  #52 (line in Coconut source)
    from typing import TYPE_CHECKING  #52 (line in Coconut source)
else:  #52 (line in Coconut source)
    try:  #52 (line in Coconut source)
        TYPE_CHECKING = _coconut.typing.TYPE_CHECKING  #52 (line in Coconut source)
    except _coconut.AttributeError as _coconut_imp_err:  #52 (line in Coconut source)
        raise _coconut.ImportError(_coconut.str(_coconut_imp_err))  #52 (line in Coconut source)
if _coconut.typing.TYPE_CHECKING:  #52 (line in Coconut source)
    from typing import Any  #52 (line in Coconut source)
else:  #52 (line in Coconut source)
    try:  #52 (line in Coconut source)
        Any = _coconut.typing.Any  #52 (line in Coconut source)
    except _coconut.AttributeError as _coconut_imp_err:  #52 (line in Coconut source)
        raise _coconut.ImportError(_coconut.str(_coconut_imp_err))  #52 (line in Coconut source)

if TYPE_CHECKING:  #54 (line in Coconut source)
    from yggdrasil_lm.core.executor import ExecutionContext  #55 (line in Coconut source)
    from yggdrasil_lm.core.executor import TraceEvent  #55 (line in Coconut source)
...
_SERVICE_NAME = "yggdrasil"  #62 (line in Coconut source)
_INSTRUMENTATION = "exporters.otel"  #63 (line in Coconut source)
_SCHEMA_URL = "https://opentelemetry.io/schemas/1.26.0"  #64 (line in Coconut source)
...
def _to_ns(dt: datetime) -> int:  #71 (line in Coconut source)
    """Convert a datetime to nanoseconds since epoch (required by OTel API)."""  #72 (line in Coconut source)
    return _coconut_tail_call(int, dt.timestamp() * 1000000000)  #73 (line in Coconut source)



@_coconut_tco  #76 (line in Coconut source)
def _end_ns(dt: datetime, duration_ms: int | None) -> int:  #76 (line in Coconut source)
    """Compute end time in ns given a timestamp and optional duration."""  #77 (line in Coconut source)
    if duration_ms is not None:  #78 (line in Coconut source)
        return _to_ns(dt) + duration_ms * 1000000  #79 (line in Coconut source)
    return _coconut_tail_call(_to_ns, dt)  #80 (line in Coconut source)
...
def _trace_id_from_session(session_id: str) -> int:  #87 (line in Coconut source)
    """Derive a stable 128-bit OTel trace ID from the session UUID."""  #88 (line in Coconut source)
    try:  #89 (line in Coconut source)
        return uuid.UUID(session_id).int  #90 (line in Coconut source)
    except ValueError:  #91 (line in Coconut source)
        return uuid.uuid5(uuid.NAMESPACE_DNS, session_id).int  #92 (line in Coconut source)



def _span_id_from_event(event_id: str) -> int:  #95 (line in Coconut source)
    """Derive a stable 64-bit OTel span ID from a TraceEvent UUID."""  #96 (line in Coconut source)
    try:  #97 (line in Coconut source)
        return uuid.UUID(event_id).int & 0xFFFFFFFFFFFFFFFF  #98 (line in Coconut source)
    except ValueError:  #99 (line in Coconut source)
        return uuid.uuid5(uuid.NAMESPACE_DNS, event_id).int & 0xFFFFFFFFFFFFFFFF  #100 (line in Coconut source)
...
def export_trace(ctx: "ExecutionContext", tracer: Any=None, service_name: str=_SERVICE_NAME,) -> None:  #107 (line in Coconut source)
    ...
    try:  #123 (line in Coconut source)
        from opentelemetry import trace as otel_trace  #124 (line in Coconut source)
        from opentelemetry.sdk.trace import TracerProvider  #125 (line in Coconut source)
        from opentelemetry.trace import NonRecordingSpan  #126 (line in Coconut source)
        from opentelemetry.trace import SpanContext  #126 (line in Coconut source)
        from opentelemetry.trace import TraceFlags  #126 (line in Coconut source)
    except ImportError:  #127 (line in Coconut source)
        import sys  #128 (line in Coconut source)
        print("[yggdrasil] opentelemetry-sdk not installed \u2014 trace export skipped. Run: pip install 'yggdrasil[observe]'", file=sys.stderr)  #129 (line in Coconut source)
        return  #134 (line in Coconut source)

    if tracer is None:  #136 (line in Coconut source)
        tracer = otel_trace.get_tracer(_INSTRUMENTATION, schema_url=_SCHEMA_URL)  #137 (line in Coconut source)

    events = ctx.trace  #139 (line in Coconut source)
    session_id = ctx.session_id  #140 (line in Coconut source)
    query = ctx.query  #141 (line in Coconut source)

# Build parent→children index
    children: dict[str | None, list[TraceEvent]] = {}  #144 (line in Coconut source)
    for e in events:  #145 (line in Coconut source)
        children.setdefault(e.parent_event_id, []).append(e)  #146 (line in Coconut source)

# Derive OTel trace ID from session ID so all spans share the same trace
    trace_id = _trace_id_from_session(session_id)  #149 (line in Coconut source)

# Render top-level events (hops)
    for root_event in children.get(None, []):  #152 (line in Coconut source)
        _export_hop(tracer, root_event, children, trace_id=trace_id, query=query)  #153 (line in Coconut source)
...
def _export_hop(tracer: Any, hop: "TraceEvent", children: dict[str | None, list["TraceEvent"]], trace_id: int, query: str,) -> None:  #163 (line in Coconut source)
    """Create one root OTel span per hop and recurse into its children."""  #170 (line in Coconut source)
    from opentelemetry.trace import SpanContext  #171 (line in Coconut source)
    from opentelemetry.trace import TraceFlags  #171 (line in Coconut source)
    from opentelemetry.trace import NonRecordingSpan  #171 (line in Coconut source)
    from opentelemetry.sdk.trace import ReadableSpan  #172 (line in Coconut source)
    from opentelemetry.trace import SpanKind  #173 (line in Coconut source)
    from opentelemetry.trace import use_span  #173 (line in Coconut source)

    p = hop.payload  #175 (line in Coconut source)
    hop_num = p.get("hop", "")  #176 (line in Coconut source)
    span_name = "hop {_coconut_format_0} — {_coconut_format_1}".format(_coconut_format_0=(hop_num), _coconut_format_1=(hop.node_name)) if hop_num else hop.node_name  #177 (line in Coconut source)

    hop_children = children.get(hop.event_id, [])  #179 (line in Coconut source)

# Compute hop span time from children when hop.duration_ms is not set
    start_ns, end_ns = _hop_times(hop, hop_children)  #182 (line in Coconut source)

    hop_attrs: dict[str, Any] = {"hop": hop_num, "node_type": p.get("node_type", ""), "node_name": hop.node_name, "session_id": hop.session_id, "query": query}  #184 (line in Coconut source)

    span_ctx = SpanContext(trace_id=trace_id, span_id=_span_id_from_event(hop.event_id), is_remote=False, trace_flags=TraceFlags(TraceFlags.SAMPLED))  #192 (line in Coconut source)
    parent_span = NonRecordingSpan(span_ctx)  #198 (line in Coconut source)

    with tracer.start_as_current_span(span_name, context=_ctx_with_span(parent_span), start_time=start_ns, attributes=hop_attrs) as hop_span:  #200 (line in Coconut source)
        hop_span.set_attribute("summary", p.get("summary", "")[:500])  #206 (line in Coconut source)

        for child in hop_children:  #208 (line in Coconut source)
            if child.event_type == "agent_start":  #209 (line in Coconut source)
                _export_agent(tracer, child, children, trace_id=trace_id)  #210 (line in Coconut source)

# End at the computed time
        hop_span.end(end_time=end_ns)  #213 (line in Coconut source)
...
def _export_agent(tracer: Any, start_ev: "TraceEvent", children: dict[str | None, list["TraceEvent"]], trace_id: int,) -> None:  #220 (line in Coconut source)
    """Create one OTel span covering the full agent execution."""  #226 (line in Coconut source)
    p = start_ev.payload  #227 (line in Coconut source)
    agent_name = start_ev.node_name  #228 (line in Coconut source)
    model = p.get("model", "unknown")  #229 (line in Coconut source)

    agent_children = children.get(start_ev.event_id, [])  #231 (line in Coconut source)

# Find agent_end to get end time
    end_event = next((e for e in agent_children if e.event_type == "agent_end"), None)  #234 (line in Coconut source)
    start_ns = _to_ns(start_ev.timestamp)  #237 (line in Coconut source)
    end_ns = _to_ns(end_event.timestamp) if end_event else start_ns + 1000000  #238 (line in Coconut source)

    agent_attrs: dict[str, Any] = {"agent.name": agent_name, "agent.model": model, "agent.tools": json.dumps(p.get("tools", [])), "agent.context": json.dumps(p.get("context", [])), "agent.query": p.get("query", "")[:500], "llm.system": "anthropic", "llm.request.model": model, "gen_ai.system": "anthropic", "gen_ai.request.model": model}  #240 (line in Coconut source)

    if end_event:  #252 (line in Coconut source)
        agent_attrs["agent.intent"] = end_event.payload.get("intent", "")  #253 (line in Coconut source)
        agent_attrs["agent.iterations"] = end_event.payload.get("iterations", 1)  #254 (line in Coconut source)
        agent_attrs["agent.summary"] = end_event.payload.get("text_summary", "")[:500]  #255 (line in Coconut source)

    with tracer.start_as_current_span("agent: {_coconut_format_0}".format(_coconut_format_0=(agent_name)), start_time=start_ns, attributes=agent_attrs) as agent_span:  #257 (line in Coconut source)
# Emit non-span child events as OTel Events
        for child in agent_children:  #263 (line in Coconut source)
            _coconut_case_match_to_0 = child.event_type  #264 (line in Coconut source)
            _coconut_case_match_check_0 = False  #264 (line in Coconut source)
            if _coconut_case_match_to_0 == "context_inject":  #264 (line in Coconut source)
                _coconut_case_match_check_0 = True  #264 (line in Coconut source)
            if _coconut_case_match_check_0:  #264 (line in Coconut source)
                agent_span.add_event("context_inject", attributes={"context.names": json.dumps(child.payload.get("context_names", [])), "context.count": child.payload.get("count", 0)}, timestamp=_to_ns(child.timestamp))  #266 (line in Coconut source)
            if not _coconut_case_match_check_0:  #274 (line in Coconut source)
                if _coconut_case_match_to_0 == "routing":  #274 (line in Coconut source)
                    _coconut_case_match_check_0 = True  #274 (line in Coconut source)
                if _coconut_case_match_check_0:  #274 (line in Coconut source)
                    cp = child.payload  #275 (line in Coconut source)
                    route_attrs: dict[str, Any] = {"routing.intent": cp.get("intent", ""), "routing.next_node_id": cp.get("next_node_id") or "__END__"}  #276 (line in Coconut source)
                    if cp.get("confidence") is not None:  #280 (line in Coconut source)
                        route_attrs["routing.confidence"] = cp["confidence"]  #281 (line in Coconut source)
                    agent_span.add_event("routing", attributes=route_attrs, timestamp=_to_ns(child.timestamp))  #282 (line in Coconut source)
            if not _coconut_case_match_check_0:  #287 (line in Coconut source)
                if _coconut_case_match_to_0 == "tool_call":  #287 (line in Coconut source)
                    _coconut_case_match_check_0 = True  #287 (line in Coconut source)
                if _coconut_case_match_check_0:  #287 (line in Coconut source)
                    _export_tool(tracer, child, children, trace_id=trace_id)  #288 (line in Coconut source)

        agent_span.end(end_time=end_ns)  #290 (line in Coconut source)
...
def _export_tool(tracer: Any, call_ev: "TraceEvent", children: dict[str | None, list["TraceEvent"]], trace_id: int,) -> None:  #297 (line in Coconut source)
    """Create one OTel span covering one tool invocation."""  #303 (line in Coconut source)
    from opentelemetry.trace.status import Status  #304 (line in Coconut source)
    from opentelemetry.trace.status import StatusCode  #304 (line in Coconut source)

    p = call_ev.payload  #306 (line in Coconut source)
    tool_name = call_ev.node_name  #307 (line in Coconut source)
    inp = p.get("input", {})  #308 (line in Coconut source)

    result_children = children.get(call_ev.event_id, [])  #310 (line in Coconut source)
    result_ev = next((e for e in result_children if e.event_type == "tool_result"), None)  #311 (line in Coconut source)

    start_ns = _to_ns(call_ev.timestamp)  #315 (line in Coconut source)
    end_ns = _to_ns(result_ev.timestamp) if result_ev else start_ns + 1000000  #316 (line in Coconut source)

    tool_attrs: dict[str, Any] = {"tool.name": tool_name, "tool.callable_ref": p.get("callable_ref", ""), "tool.input": json.dumps(inp)[:1000]}  #318 (line in Coconut source)

    success = True  #324 (line in Coconut source)
    if result_ev:  #325 (line in Coconut source)
        rp = result_ev.payload  #326 (line in Coconut source)
        success = rp.get("success", True)  #327 (line in Coconut source)
        tool_attrs["tool.output_summary"] = rp.get("output_summary", "")[:500]  #328 (line in Coconut source)
        tool_attrs["tool.success"] = success  #329 (line in Coconut source)

    with tracer.start_as_current_span("tool: {_coconut_format_0}".format(_coconut_format_0=(tool_name)), start_time=start_ns, attributes=tool_attrs) as tool_span:  #331 (line in Coconut source)
        if not success:  #336 (line in Coconut source)
            tool_span.set_status(Status(StatusCode.ERROR))  #337 (line in Coconut source)

        tool_span.end(end_time=end_ns)  #339 (line in Coconut source)
...
def _hop_times(hop: "TraceEvent", hop_children: list["TraceEvent"],) -> tuple[int, int]:  #346 (line in Coconut source)
    """Compute start/end nanoseconds for a hop span."""  #350 (line in Coconut source)
    if hop.duration_ms is not None:  #351 (line in Coconut source)
        start = _to_ns(hop.timestamp)  #352 (line in Coconut source)
        return start, start + hop.duration_ms * 1000000  #353 (line in Coconut source)

    if hop_children:  #355 (line in Coconut source)
        timestamps = (list)((map)(lambda e: _to_ns(e.timestamp), hop_children))  #356 (line in Coconut source)
        start_ns = min(timestamps)  #357 (line in Coconut source)

        agent_ends = [e for e in hop_children if e.event_type == "agent_end" and e.duration_ms]  #359 (line in Coconut source)
        if agent_ends:  #363 (line in Coconut source)
            end_ns = ((max)((map)(lambda e: _to_ns(e.timestamp) + e.duration_ms * 1000000, agent_ends)))  #364 (line in Coconut source)
        else:  #367 (line in Coconut source)
            end_ns = max(timestamps) + 1000000  # +1ms fallback  #368 (line in Coconut source)

        return start_ns, end_ns  #370 (line in Coconut source)

    ts = _to_ns(hop.timestamp)  #372 (line in Coconut source)
    return ts, ts + 1000000  #373 (line in Coconut source)



@_coconut_tco  #376 (line in Coconut source)
def _ctx_with_span(span: Any) -> Any:  #376 (line in Coconut source)
    """Return an OTel Context that sets the given span as current."""  #377 (line in Coconut source)
    from opentelemetry import context as otel_context  #378 (line in Coconut source)
    from opentelemetry.trace import set_span_in_context  #379 (line in Coconut source)
    return _coconut_tail_call(set_span_in_context, span)  #380 (line in Coconut source)
