...
from yggdrasil_lm import GraphApp, create_agent, create_context, create_prompt, create_tool
...
from yggdrasil_lm.retrieval import semantic_search
...
def test_helper_constructors_create_nodes():
    ...
    tool = create_tool("echo", callable_ref="tools.echo.echo")
    context = create_context("C", "content")
    prompt = create_prompt("P", "You are helpful.")

    assert agent.name == "A"
    assert tool.callable_ref == "tools.echo.echo"
